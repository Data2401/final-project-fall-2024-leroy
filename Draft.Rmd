...
# Has USA won the FIFA World Cup?

```{r}
worldcups %>% 
  filter(winner  == "USA") %>% knitr::kable()
...
W <- worldcups %>% 
  ...
  aes(x = winner,
      fill = winner) +
  geom_bar() +
  labs(title = "Championship Winner")
...
wcmatches %>%
  select(year, stage, winning_team, Total_Gols)
...
# Three different Germany Teams?

```{r}
inner_join(set1, set2, join_by("home_team" == "away_team")) %>% 
  mutate(total_goals = home_score + away_score) %>%
  select(team = home_team, total_goals) %>%
  mutate(team = recode(team, 'West Germany' = 'Germany', 'East Germany' = 'Germany', 'Germany' = 'Germany')) %>% 
  
  arrange(-total_goals)

```

# Has any matches end in extra time or penalties?
```{r}
wcmatches %>%
  na.omit() %>% 
  select(year, home_team, away_team, win_conditions) %>% knitr::kable()
...
wcmatches %>% 
  ...
  select(year, stage, home_team, away_team, outcome) %>% knitr::kable()
```
