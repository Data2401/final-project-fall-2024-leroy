...
This data collected from [Kaggle](https://www.kaggle.com/datasets/evangower/fifa-world-cup).

What's in the data? Fun Facts? Can the next FIFA World Cup winner be predicted? 

# Which Countries have hosted the FIFA World Cup?
```{r}
worldcups %>% 
  distinct(host) %>% knitr::kable()
```
...
```{r}
worldcups %>% 
  distinct(winner, year) %>% knitr::kable()
